<?php 
	...
	$html->addCrumb($name, '/' . $controller  . '/dashboard/' . $doc['Doc']['table_id']); 
	$html->addCrumb('Documents', '/docs/' . $doc['Doc']['table_type'] . '/' . $doc['Doc']['table_id']); 
	$html->addCrumb($doc['Doc']['title'], '/docs/view/' . $doc['Doc']['id']); 
	$html->addCrumb('Document Versions', '/docs/versions/' . $doc['Doc']['id']); 
?>
<div id="version-div"></div>
<script type="text/javascript">
	laboratree.context = <?php echo $javascript->object($context); ?>;
	laboratree.docs.makeVersions('version-div', '<?php echo $html->url('/docs/versions/' . $doc['Doc']['id'] . '.json'); ?>');
</script>
